...
async def api_process_image(file: UploadFile = File(...)):
    ...
    frame = cv2.imdecode(arr, cv2.IMREAD_COLOR)
    if frame is None:
        return {"error": "Could not decode image", "letter": None, "confidence": None, "word": "", "sentence": ""}
    _, _committed, confidence, letter, word, sentence = process_frame(frame)
    return {"letter": letter, "confidence": confidence, "word": word, "sentence": sentence}
